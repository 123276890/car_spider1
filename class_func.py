...
class func(object):
    ...
    # 价格换算 万元->元
    def conversion_price(p, isWan=False):
        if type(p) == str and isWan == False:
            # 暂无
            if p.count("暂无") > 0:
               p = "暂无"
            # 厂商指导价：6.58 -> 6.58万元
        ...
        # 6.58 -> 65800
            ...
            if p.count('厂商') > 0 and isWan == True:
                p = p.split("：")
                p = p[1].replace('万', '')
                p = int((float(p) + 10 ** -5) * 10000)
            else:
    ...
    # schemes金融方案
    def json_schemes(period, first_price, month_price, first_ratio=0, final=''):
        if final != '':

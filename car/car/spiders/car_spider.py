...
class CarSpider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        for lett in letters:
            logging.debug('Brand initials：%s' % lett)
    ...
    def parse(self, response):
        logging.debug('当前访问的品牌列表页面：%s' % response.url)
    ...
    def parse_article(self, response):
        logging.debug('当前访问的车系页面：%s' % response.url)
    ...
    def parse_article_detail(self, response):
        logging.debug('当前访问的车子页面：%s' % response.url)
    ...
    def parse_article_config(self, response):
        ...
        for info in infos:
            ...
            logging.debug('当前访问车的详细配置页面：' + response.url)
            logging.debug('车ID：' + str(item['type_id']))
            logging.debug('车品牌： ' + str(item['brand_name']))
            logging.debug('车系：' + str(item['series_name']))
            logging.debug('车名：' + str(item['car_name']))
            logging.debug('价格：' + str(item['market_price_str']))
